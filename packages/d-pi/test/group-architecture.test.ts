...
function captureTool(): ToolDefinition & { execute: (id: string, params: unknown) => Promise<unknown> } {
	const tool = createGroupArchitectureTool(makeChannel().channel);
	return tool as unknown as ToolDefinition & { execute: (id: string, params: unknown) => Promise<unknown> };
}
...
describe("group_architecture tool", () => {
	...
	it("execute() posts a tool_call for group_architecture and renders the snapshot", async () => {
		...
		const executePromise = (tool as unknown as {
