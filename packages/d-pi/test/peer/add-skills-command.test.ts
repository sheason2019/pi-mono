...
import { runAddSkills } from "../../src/peer/commands/add-skills.js";
...
describe("d-pi peer add-skills", () => {
	it("installs the built-in agent guidance skills into the peer workspace .pi skills directory", () => {
		const cwd = mkdtempSync(join(tmpdir(), "pi-peer-add-skills-"));
		tempDirs.push(cwd);
		...
		expect(configContent).toContain("d-pi peer");
		expect(configContent).toContain("d-pi hub");
		expect(configContent).toContain("Peer source lookup order");
