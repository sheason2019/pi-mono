...
export function createReloadTools(deps: ReloadToolsDeps): ToolDefinition {
	return defineTool({
		...
		description:
			"Reload d-pi resources (skills, system prompt, AGENTS.md / CLAUDE.md context files, extensions) at runtime without restarting the hub or worker. Also re-reads ~/.pi/agent/models.json so newly added providers / models become available in the same call, and re-reads the workspace-level APPEND_SYSTEM.md plus this agent's agent.ts identity so changes to the ## Agent identity section (description / roles / model name / tool allow-deny list) take effect. Returns a JSON snapshot of the post-reload state so the caller can verify what changed. Takes effect on the next agent turn — the in-flight turn is aborted. Does NOT re-parse agents/<name>/agent.ts for non-identity fields that affect hub-level wiring (parent import changes, agent rename, session directory layout — these require a hub restart) and does NOT re-read team-template role directories beyond the AGENTS.md / skills / extensions already wired in (require destroy + recreate or hub restart).",
