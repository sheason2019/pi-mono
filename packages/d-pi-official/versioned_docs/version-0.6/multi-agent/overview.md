...
| [group_architecture](./group-architecture) |查整个 agent树 |
| [destroy_agent](./destroy-agent) |收尾时销毁子 agent |
| [meta-connect](./meta-connect) | 在 TUI输入 `connect <id>`把消息定向到指定 agent |

##角色（Roles）

角色（roles）— 见 docs/group-architecture/roles（v0.7计划引入）是一类任务的可复用预设：把"指令、skill、extension"打成模板，需要时给子 agent套上。
...
roles的目录布局在 `group-architecture/roles/<name>/`下，详见 roles文档 — 见 docs/group-architecture/roles（v0.7计划引入）。
...
1. root: create_agent(name="researcher", roles=["researcher"])
 →子 agent进入 starting → ready状态，继承 group-architecture/roles/researcher/下的资源
...
5. root: group_architecture()查整个树确认两个子 agent状态
...
「上下文工程」是把可复用的指令、skill、extension预置到 `group-architecture/`目录的工程实践——这是 d-pi多 agent系统的**作者合约**。

详见 Group Architecture →概览 — 见 docs/group-architecture/overview（v0.7计划引入）。

## 相关

- [Sources概览](../sources/overview) — agent也可以订阅外部数据源
- [Remote Execution概览](../remote-execution/overview) — agent调的工具跑在哪里
- 角色Roles — 见 docs/group-architecture/roles（v0.7计划引入） —可复用的 agent预设模板
