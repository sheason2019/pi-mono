...
一句话：`agent-network/`下的文件系统布局就是 d-pi 多 agent系统的**作者合约**——你照这个布局放文件，d-pi会在 hub启动或子 agent 创建时按规则加载到对应 agent 的上下文里。
...
这些「可复用的上下文」必须存在某个地方。**`agent-network/` 就是 d-pi规定这个地方的格式**。
...
- **不是运行时自动生成**——`d-pi init`不会创建 `agent-network/`，完全靠你**手动放**
...
├── agent-network/ # ← 这就是约定目录（在 workspace根，**不**在 .dpi/ 下）
│ ├── AGENTS.md # 网络级共享上下文
│ ├── skills/ # 网络级 skill池
...
| **Network 级** | `agent-network/{AGENTS.md, skills/, extensions/}` | 项目作者 | hub启动 + 子 agent 创建 | 所有 agent（root 自动，子 agent显式） |
| **Role 级** | `agent-network/roles/<name>/{AGENTS.md, skills/, extensions/}` | 项目作者 |引用了此 role 的 agent启动时 |套了 role 的 agent |
...
2. **Network 级** `agent-network/AGENTS.md` + `agent-network/skills/` + `agent-network/extensions/`
3. **Role 级** `agent-network/roles/<each effective role>/{AGENTS.md, skills/, extensions/}`
4. **Workspace顶级** `skills/` + `extensions/`（根目录，不在 `agent-network/` 下也不在 `.dpi/` 下）
...
- [agent_network工具](../multi-agent/agent-network) —— agent视角的查询入口
