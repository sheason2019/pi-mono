...
一句话：role 是 `agent-network/roles/<name>/`下的一个目录，里面装「指令 + skill + extension」，给子 agent套上即用。**它是 Agent Network约定下的子概念**——`directory-convention`讲目录怎么放，本页讲 role怎么写。

## 一个 role长什么样

```
agent-network/roles/researcher/
├── AGENTS.md # 该角色的指令
├── skills/ # 该角色专属 skill
│ └── git-bisect/
│ └── SKILL.md
...
- **root agent**（`agentName === "root"`）的 effective roles：
 ...
 - **结果**：`agent-network/roles/root/` 如果存在，root **自动**加载它——**不需要**在 `agents/root/agent.json` 里写 `"roles": ["root"]`
 - 如果 `agent-network/roles/root/` 不存在，静默跳过（不报错）
...
```bash
mkdir -p agent-network/roles/researcher/{skills,extensions}
```
...
写 `agent-network/roles/researcher/skills/git-bisect/SKILL.md`：
...
写 `agent-network/roles/researcher/extensions/auto-cite.ts`：
...
d-pi serve &
sleep3
# root启动时 hub会读 agent-network/，有错会打 stderr
ls agent-network/roles/
cat agent-network/roles/researcher/AGENTS.md
...
6. **空 role浪费**——`agent-network/roles/empty/` 空目录占个位置。删了或至少放个 README说明这是 placeholder
...
|症状 |原因 |修法 |
|---|---|---|
| `Unknown agent role "foo"` | `agent-network/roles/foo/` 不存在 |确认目录拼写 +拼写对得上（`kebab-case` vs `camelCase`） |
