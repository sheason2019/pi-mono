...
[角色（roles）](../agent-network/roles)是一类任务的可复用预设：把"指令、skill、extension"打成模板，需要时给子 agent套上。
...
roles的目录布局在 `agent-network/roles/<name>/`下，详见 [roles文档](../agent-network/roles)。
...
- [角色Roles](../agent-network/roles) —可复用的 agent预设模板
