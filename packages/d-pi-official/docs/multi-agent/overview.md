...
[角色（roles）](../group-architecture/roles)是一类任务的可复用预设：把"指令、skill、extension"打成模板，需要时给子 agent套上。
...
roles的目录布局在 `group-architecture/roles/<name>/`下，详见 [roles文档](../group-architecture/roles)。
