...
**Source:** [`@sheason/pi-tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@sheason/pi-tui";
...
```typescript
import { Container, type Focusable, Input } from "@sheason/pi-tui";
...
Import from `@sheason/pi-tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@sheason/pi-tui";
```
...
```typescript
import { matchesKey, Key } from "@sheason/pi-tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@sheason/pi-tui";
...
For letting users pick from a list of options. Use `SelectList` from `@sheason/pi-tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@sheason/pi-coding-agent";
import { DynamicBorder } from "@sheason/pi-coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@sheason/pi-tui";
...
For toggling multiple settings. Use `SettingsList` from `@sheason/pi-tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@sheason/pi-coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@sheason/pi-tui";
...
import { matchesKey, truncateToWidth } from "@sheason/pi-tui";
