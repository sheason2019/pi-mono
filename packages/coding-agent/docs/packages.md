...
Pi bundles core packages for extensions and skills. If you import any of these, list them in `peerDependencies` with a `"*"` range and do not bundle them: `@sheason/pi-ai`, `@earendil-works/pi-agent-core`, `@sheason/pi-coding-agent`, `@earendil-works/pi-tui`, `typebox`.
