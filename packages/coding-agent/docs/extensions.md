...
| Package | Purpose |
|---------|---------|
| `@sheason/pi-coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@sheason/pi-ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@sheason/pi-tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@sheason/pi-coding-agent";
...
```typescript
import { isBashToolResult } from "@sheason/pi-coding-agent";
...
```typescript
import { createLocalBashOperations } from "@sheason/pi-coding-agent";
...
```typescript
import type { AutocompleteItem } from "@sheason/pi-tui";
...
```typescript
import { withFileMutationQueue } from "@sheason/pi-coding-agent";
...
**Important:** Use `StringEnum` from `@sheason/pi-ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@sheason/pi-coding-agent";
...
```typescript
import { createBashTool } from "@sheason/pi-coding-agent";
...
```typescript
import { keyHint } from "@sheason/pi-coding-agent";
...
```typescript
import { Text, Component } from "@sheason/pi-tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@sheason/pi-coding-agent";
import { matchesKey } from "@sheason/pi-tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@sheason/pi-coding-agent";
